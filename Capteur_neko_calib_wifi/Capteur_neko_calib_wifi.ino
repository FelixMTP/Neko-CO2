...
// Paramètres Wifi :
const char* ssid = "Nom du réseau Wifi";            // Entrer ici le SSID du réseau wifi auquel le capteur doit se connecter. Laisser les " "
const char* pwd = "mot de passe du reseau wifi";    // Entrer ici le mot de passe du réseau wifi auquel le capteur doit se connecter. Laisser les " "

// Paramètres de calibration 
String cal_passwort = "test"; // Mot de passe de calibration

// Paramètres à modifier en fonction de votre montage :
...
int valeur_calib = 400; // Mettre ici le taux de CO2 servant à la calibration à taux forcé
...
int input_valcalib = 430; //valeur de calibration par défaut
void CO2_Kalibrierfunktion(int valeur){ // Kalibrierfunktion
// Forced Calibration Sensirion SCD 30
  ...
  airSensorSCD30.setForcedRecalibrationFactor(valeur); // fresh air 
  Serial.println(" done");
}
...
const char INDEX_HTML_SUBMIT[] =
     ...
     "<P>"
        ...
        "<INPUT type=\"text\" name=\"valcalib\">"
        "<INPUT type=\"submit\" value=\"Calibrate\">"
      "</P>"
     "</FORM>";

String cal_message  = "Entrez le mot de passe de calibration";      // Nachricht
...
String messwertTabelle(){ 
  ...
  String nam1="Temperature";
  String nam2="Humidite";
  String nam3="";
...
void serverHomepage() { 
 if (server.hasArg("message")) {// Wenn Kalibrierpasswort eingetroffen,
   ...
   input_valcalib = server.arg("valcalib").toInt();     // dann Text vom Client einlesen

   if (input == cal_passwort){
      cal_message = "Calibration en cours (patientez environ 20s)...";
      doCal = 1;
   } else
      cal_message = "Mauvais mot de passe";
 } else cal_message = "";
...
void setup() {
  ...
  Serial.println("\n");
  WiFi.begin(ssid, pwd);  // Initialisation du Wifi
  ...
  server.on("/", serverHomepage);  // Chargement de la page d'accueil
  server.begin();  // Initialisation du serveur web
  Serial.println("Serveur web actif");

  // Initialize the SCD30 driver
  Serial.println("Initialisation du driver");
  Wire.begin(SDA_pin, SCL_pin);
  
  // Initialize sensor
  Serial.println("Initialisation du sensor");
  if (airSensorSCD30.begin() == false) {
    Serial.println("The SCD30 did not respond. Please check wiring.");
    while(1) {yield(); delay(1);}
  }

  //désactivation de l'autocalibration
  Serial.println("Désactivation de l'autocalibration");
  airSensorSCD30.setAutoSelfCalibration(false); // Sensirion no auto calibration
  airSensorSCD30.setMeasurementInterval(2000);     // CO2-Messung alle 5 s
}
...
// EasyBuzzer.update();
  ...
  if (CO2 < 430){       // S'il y a moins de 430 ppm de CO2
  ...
  else if (CO2 < 800 && CO2 >= 430){     // S'il y a entre 430 et 800 ppm de CO2
  ...
  else if (CO2 >= 800 && CO2 < 1500){    // S'il y a entre 800 et 1500 ppm de CO2
    digitalWrite(ledB,LOW);
  ...
  else if (CO2 >= 1500){    // Si on dépasse les 1500 ppm de CO2
    digitalWrite(ledB,LOW);
...
void webpage() 
{
  ...
  delay(1);
  if (doCal) {
     Serial.print("Calibration à "); Serial.println(input_valcalib);
     CO2_Kalibrierfunktion(input_valcalib); // Kalibrierfunktion aufrufen 
     doCal=0; 
  }
}
